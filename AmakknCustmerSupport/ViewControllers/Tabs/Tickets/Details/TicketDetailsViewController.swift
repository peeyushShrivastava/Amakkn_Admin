...
class TicketDetailsViewController: UIViewController {
    ...
    @IBOutlet weak var ibUserInfoView: UIView!
    @IBOutlet weak var ibUserVerifiedIcon: UIImageView!
    @IBOutlet weak var ibUserName: UILabel!
    @IBOutlet weak var ibUserType: UILabel!
    ...
    private func updateUserInfo() {
        ibUserInfoView.layer.masksToBounds = true
        ibUserInfoView.layer.borderWidth = 1.0
        ibUserInfoView.layer.borderColor = AppColors.borderColor?.cgColor

        ibUserName.text = viewModel.getUserName()
        ibUserType.text = viewModel.getUserType()
        ibUserAvatar.sd_setImage(with: URL(string: viewModel.getUserAvatar()), placeholderImage: UIImage(named: "icAvatarPlaceHolderIcon"))

        ibUserVerifiedIcon.isHidden = !viewModel.isUserVerified
    }
...
extension TicketDetailsViewController {
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "createTicketSegueID" {
...
extension TicketDetailsViewController {
    @IBAction func callButtonTapped(_ sender: UIButton) {
        viewModel.call { [weak self] errorStr in
            DispatchQueue.main.async {
                self?.showAlert(for: errorStr)
            }
        }
    }

    @IBAction func userInfoTapped(_ sender: UIButton) {
    ...
    private func pushAddViolationVC() {
        guard let violationVC =  CreateViolationViewController.instantiateSelf() else { return }

        violationVC.viewModel.updateTicketID(viewModel.getTicketID())
        violationVC.viewModel.updateUserInfo(viewModel.getUserInfo())

        navigationController?.pushViewController(violationVC, animated: true)
    }
}
